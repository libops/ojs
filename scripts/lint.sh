...
docker run --rm \
  ...
  -lc '
    ...
    if [ ! -d rootfs/var/www/ojs ]; then
      echo "No custom OJS application overlay found; skipping OJS PHP lint."
      exit 0
    fi

    if ! find rootfs/var/www/ojs -type f \( -name "*.php" -o -name "*.inc.php" \) | grep -q .; then
      echo "No custom OJS PHP files found; skipping OJS PHP lint."
      exit 0
    fi

    find rootfs/var/www/ojs -type f \( -name "*.php" -o -name "*.inc.php" \) -exec php -l {} \;
  '
